...
func TestIsRSAE(t *testing.T) {
	tests := []struct {
		...
		{"RSA_PSS_RSAE_SHA384", RSA_PSS_RSAE_SHA384, true},
		{"RSA_PSS_RSAE_SHA512", RSA_PSS_RSAE_SHA512, true},
		{"RSA_PSS_PSS_SHA256", RSA_PSS_PSS_SHA256, false},
		{"RSA_PSS_PSS_SHA384", RSA_PSS_PSS_SHA384, false},
		{"RSA_PSS_PSS_SHA512", RSA_PSS_PSS_SHA512, false},
		{"RSA", RSA, false},
		{"ECDSA", ECDSA, false},
		// Edge case: 0x0807 (Ed25519) falls within the 0x0804-0x0806 range
		// but is NOT an RSAE scheme. This test ensures we check specific values, not just ranges.
		{"0x0807_Ed25519_raw", Algorithm(0x0807), false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.alg.IsRSAE()
			assert.Equal(t, tt.expected, result)
		})
	}
}
